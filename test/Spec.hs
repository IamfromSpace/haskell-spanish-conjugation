...
main =
    hspec $ do
        ...
        describe "not replace an unstressed i with a y between a silent vowels" $ do
            it "0" $ delinquir "ió" `shouldBe` Right (Just "delinquió")
            it "1" $ delinquir "ieron" `shouldBe` Right (Just "delinquieron")
            it "2" $ distinguir "ió" `shouldBe` Right (Just "distinguió")
            it "3" $ distinguir "ieron" `shouldBe` Right (Just "distinguieron")
        ...
        describe
            ...
            it "durmiendo" $ dormir "iendo" `shouldBe` Right (Just "durmiendo")
            it "durmieron" $ dormir "ieron" `shouldBe` Right (Just "durmieron")
            it "sintamos" $ sentir "amos" `shouldBe` Right (Just "sintamos")
            it "sintiendo" $ sentir "iendo" `shouldBe` Right (Just "sintiendo")
            it "sintieron" $ sentir "ieron" `shouldBe` Right (Just "sintieron")
        describe
